...
comp3 :: (MonadCont m, MonadState (Int, Int) m, MonadIO m) => m ()
comp3 = do
          liftIO $ putStrLn "start"
          for (return ()) ((<10) <$> gets fst) (modify $ \(i, j) -> (i+1, j)) $ \outer -> do
            i <- gets fst                                   
            liftIO $ putStrLn ("i=" ++ show i)        
            for (modify $ \(i, j) -> (i, 0)) ((<10) <$> gets snd) (modify $ \(i, j) -> (i, j+1)) $ \inner -> do
              j <- gets snd                        
              liftIO $ putStrLn ("  j=" ++ show j)
              when (i == 3 && j == 4) $ do
                liftIO $ putStrLn "solution found"
